import sys, time, os
sys.path.append("../../")
from match.point_wise.model import Model
...
class Trainer(object):
    ...
    def train(self):
        ...
        for e in range(config.model_parameters.train_epoch):
            ...
            t1 = time.time()
            dev_acc, prediction = self.eval_step(self.model, sess, (self.data_dic["dev_data_x"], self.data_dic["dev_data_y"]))
    ...
    def test(self, sess):
        ...
        for x_batch, y_batch in fill_feed_dict(self.data_dic["test_data_x"], 
                                              ...
                                              config.model_parameters.batch_size):
            acc, prediction = self.eval_step(self.model, sess, (x_batch, y_batch))
            test_acc += acc 
            cnt += 1
